...
class PemeriksaanController extends Controller
{
    ...
    public function checkTodayExamination(Request $request)
    {
        try {
            $anakId = $request->anak_id;
            $today = now()->format('Y-m-d');

            $pemeriksaan = $this->pemeriksaanService->getPemeriksaanByAnakAndDate($anakId, $today);

            if ($pemeriksaan) {
                $imunisasi = $this->imunisasiService->getImunisasiByPemeriksaanId($pemeriksaan->id);
                ...
                $response = [
                    ...
                    'data' => [
                        ...
                        'status_imunisasi' => $imunisasi ? 'Diberikan' : 'Belum Diberikan',
                        'jenis_imunisasi' => $imunisasi ? $imunisasi->jenis_imunisasi->pluck('id')->toArray() : [],
                        'keterangan' => $imunisasi ? $imunisasi->keterangan : null
                    ]
                ];
            } else {
    ...
    public function store(StorePemeriksaanRequest $request)
    {
        ...
        try {
            ...
            $today = now()->format('Y-m-d');
            $existingPemeriksaan = $this->pemeriksaanService->getPemeriksaanByAnakAndDate($request->anak_id, $today);

            $updateData = array_filter($pemeriksaanData, function ($value) {
            ...
            if ($existingPemeriksaan) {
                $pemeriksaan = $this->pemeriksaanService->updatePemeriksaan($updateData, $existingPemeriksaan->id);
            ...
            if ($request->status_imunisasi === 'Diberikan' && !empty($request->jenis_imunisasi)) {
                ...
                $existingImunisasi = $this->imunisasiService->getImunisasiByPemeriksaanId($pemeriksaan->id);

                if ($existingImunisasi) {
                    $imunisasi = $this->imunisasiService->updateImunisasi($imunisasiData, $existingImunisasi->id);
            ...
            DB::commit();
            $message = $existingPemeriksaan ? 'Data Pemeriksaan Berhasil Diperbarui' : 'Data Pemeriksaan Berhasil Ditambahkan';
            return redirect()->route('pemeriksaan.index')->with('success', $message);
        } catch (\Exception $e) {
            DB::rollback();
